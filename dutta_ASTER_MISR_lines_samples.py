...
def getbls(asterfile, region):
    ...
    misrfile = glob.glob('/data/gdi/c/mdevera2/dutta_RCCM/*'+orbit+'*.hdf')[0]
...
out_dir = '/data/gdi/c/mdevera2/ASTER_MISR_dutta2/'
if not os.path.exists(out_dir): 
    os.makedirs(out_dir)

in_dir = '/data/gdi/c/mdevera2/dutta_AST_L1T/'

for i in range(1,7):
    if not os.path.exists(out_dir+'/'+str(i)+'/blocks/'): 
        os.makedirs(out_dir+'/'+str(i)+'/blocks/')
    if not os.path.exists(out_dir+'/'+str(i)+'/line/'): 
        os.makedirs(out_dir+'/'+str(i)+'/line/')
    if not os.path.exists(out_dir+'/'+str(i)+'/sample/'): 
        os.makedirs(out_dir+'/'+str(i)+'/sample/')

    file_dir = in_dir + '/'+ str(i)
    print(file_dir)
